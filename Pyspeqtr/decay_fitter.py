...
class DecayFitter:
    ...
    def __init__(self):
        self.models = {'grow_decay' : self.grow_decay,
                       ...
                       'decay_only_linbg' : self.decay_only_linbg, 
                       'decay_only2' : self.decay_only2,
                       'decay_onlyAll' : self.decay_onlyAll,
                       'decay_onlyAll2' : self.decay_onlyAll2,
    ...
    def decay_only_linbg(self, params, data_x):
        T0 = params['T0'].value
        T1 = params['T1'].value
        T2 = params['T2'].value
        P1 = params['P1'].value
        ...
        for t in data_x:
            if T0 > t:
                ...
                y.append(P1 * (numpy.exp(T1 / t1) - 1) * numpy.exp(-t / t1)+m*t1+b)
            else:
                y.append(0)
        return numpy.array(y)
    ...
    def decay_onlyAll(self, params, data_x):
        """Simplified bateman, decay part only, all activity in the chain"""
        T0 = params['T0'].value
        T1 = params['T1'].value
        T2 = params['T2'].value
        N1 = params['P1'].value
        ...
        for t in data_x:
            if  t < T0:
                ...
                y.append(N1 / (t1 - t2) * 
                        (numpy.exp(-ts / t1) - numpy.exp(-ts / t2)) +
                        N2 / t2 * numpy.exp(-ts/ t2)+N1 / t1 * numpy.exp(-ts/ t1))
            else:
                y.append(0)
        return numpy.array(y)

    def decay_onlyAll2(self, params, data_x):
        """Simplified bateman, decay part only, all activity in the chain with separate parameters for all parts"""
        T0 = params['T0'].value
        T1 = params['T1'].value
        T2 = params['T2'].value
        N1 = params['P1'].value
    ...
    def fit(self, data_x, data_y, data_dy, model, parameters):
        ...
        return (time, counts, result.params, result.redchi, parameters)
